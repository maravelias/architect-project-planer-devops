...
# cleanup.sh
# Safely remove the ArchPlan local stack, with options for local or VM (systemd-managed) deployments.
...
#   sudo bash local-dev/scripts/cleanup.sh --vm --repo-dir /opt/architect-project-planer-devops \
...
PROJECT_NAME="archplan-local-stack"
NETWORK_NAME="archplan-network"
SERVICE_NAME="archplan-local-stack.service"
...
Options:
  --repo-dir PATH         Repo root path (defaults: local=script/../.., vm=/opt/architect-project-planer-devops)
...
if [[ -z "${REPO_DIR}" ]]; then
  if [[ "${MODE}" == "local" ]]; then
    ...
    REPO_DIR="/opt/architect-project-planer-devops"
  fi
fi
