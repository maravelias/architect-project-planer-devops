...
#   sudo bash local-dev/scripts/deploy-to-vm.sh --repo-dir /opt/architect-project-planer-devops --user ubuntu --with-systemd
...
install_systemd_unit() {
  echo "==> Installing systemd unit: archplan-local-stack.service"
  SERVICE_FILE=/etc/systemd/system/archplan-local-stack.service
  cat > "${SERVICE_FILE}" <<UNIT
[Unit]
Description=rchPlan local stack (Docker Compose)
...
UNIT
  ...
  systemctl enable --now archplan-local-stack.service
}
